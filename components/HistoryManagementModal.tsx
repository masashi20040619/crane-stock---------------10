...
const HistoryManagementModal: React.FC<HistoryManagementModalProps> = ({
  ...
  const isBuyRecord = (record: HistoryRecord) => record.type === 'buy' || record.type === 'stock';
  const isSellRecord = (record: HistoryRecord) => record.type === 'sell';

  const allHistory = useMemo(() => {
    const records: { prizeName: string, prizeId: string, record: HistoryRecord }[] = [];
    prizes.forEach(prize => {
      prize.history?.forEach(h => {
        if (isBuyRecord(h) || isSellRecord(h)) {
          records.push({
    ...
    return allHistory.filter(item => {
      const typeMatch =
        filterType === 'all'
          ? true
          : filterType === 'buy'
            ? isBuyRecord(item.record)
            : isSellRecord(item.record);
        ...
        <div className="p-4 sm:p-6 bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row gap-4 shrink-0">
          ...
          <div className="flex bg-white dark:bg-slate-800 p-1 rounded-xl shrink-0">
            ...
            <button 
              ...
              className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${filterType === 'buy' ? 'bg-amber-500 text-white shadow-md' : 'text-slate-400'}`}
            >
              仕入れ
            </button>
            <button 
                ...
                filteredHistory.map(({ prizeName, prizeId, record }) => (
                  <tr key={record.id} className="border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50">
                    ...
                    <td className="px-4 py-4">
                      <span className={`px-2 py-0.5 rounded-md text-[10px] font-black uppercase ${isBuyRecord(record) ? 'bg-amber-50 text-amber-600 dark:bg-amber-900/20 dark:text-amber-400' : 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-400'}`}>
                        {isBuyRecord(record) ? '仕入れ' : '売却'}
                      </span>
                    </td>
